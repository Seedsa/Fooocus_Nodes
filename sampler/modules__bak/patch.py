...
def patched_cldm_forward(self, x, hint, timesteps, context, y=None, **kwargs):
    # 确保 advanced_parameters 不是 None 并且 controlnet_softness 有有效数值
    if advanced_parameters is not None and isinstance(advanced_parameters, dict) and 'controlnet_softness' in advanced_parameters:
        controlnet_softness = advanced_parameters['controlnet_softness']
        if controlnet_softness is not None:
            # 添加适当的类型检查，例如确保它是一个可以比较的数值类型（int 或 float）
            assert isinstance(controlnet_softness, (int, float)), "controlnet_softness must be an int or float."
        else:
            # 如果没有提供 controlnet_softness 或其值为 None，则给定一个默认值
            controlnet_softness = 0
    else:
        controlnet_softness = 0
