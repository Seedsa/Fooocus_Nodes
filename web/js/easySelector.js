...
async function getStylesImage(name, styles_name) {
  ...
  if (styles_image_cache[styles_name][name])
    return styles_image_cache[styles_name][name];
  else {
    const resp = await api.fetchApi(
      `/fooocus/prompt/styles/image?name=${name}&styles_name=${styles_name}`
    );
    if (resp.status === 200) {
      ...
      $el(
        "label.easyuse-prompt-styles-tag",
        {
          dataset: {
            tag: k["name"],
            name:
              language == "zh-CN" && k["name_cn"] ? k["name_cn"] : k["name"],
            imgName: k["imgName"],
            index: i,
          },
          $: (el) => {
            ...
            $el("div.tools", [
              $el("button.delete", {
                ...
                onclick: () => {
                  selector.element.children[1]
                    .querySelectorAll(".easyuse-prompt-styles-tag-selected")
                    .forEach((el) => {
              ...
              $el("textarea.search", {
                ...
                oninput: (e) => {
                  ...
                  selector.element.children[1]
                    .querySelectorAll(".easyuse-prompt-styles-tag")
                    .forEach((el) => {
                      if (
                        el.dataset.name.indexOf(value) != -1 ||
                        el.dataset.tag.indexOf(value) != -1 ||
                        el.classList.value.indexOf(
                          "easyuse-prompt-styles-tag-selected"
                        ) != -1
                      ) {
