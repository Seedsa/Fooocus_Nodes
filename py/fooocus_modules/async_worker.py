...
def worker():
    ...
    from fooocus_modules.sdxl_styles import apply_style, apply_wildcards, fooocus_expansion
    from fooocus_modules.private_logger import log
    from extras.expansion import safe_str
    from fooocus_modules.util import remove_empty_str, HWC3, resize_image, \
    ...
    def handler(async_task):
        ...
        if performance_selection == 'Extreme Speed':
            ...
            loras += [(fooocus_modules.config.downloading_sdxl_lcm_lora(), 1.0)]
            ...
            fooocus_modules.patch.sharpness = sharpness = 0.0
            cfg_scale = guidance_scale = 1.0
            fooocus_modules.patch.adaptive_cfg = advanced_parameters.adaptive_cfg = 1.0
            refiner_switch = 1.0
            fooocus_modules.patch.positive_adm_scale = advanced_parameters.adm_scaler_positive = 1.0
            fooocus_modules.patch.negative_adm_scale = advanced_parameters.adm_scaler_negative = 1.0
            fooocus_modules.patch.adm_scaler_end = advanced_parameters.adm_scaler_end = 0.0
            steps = 8

        fooocus_modules.patch.adaptive_cfg = advanced_parameters.adaptive_cfg
        print(f'[参数] 自适应 Adaptive CFG = {fooocus_modules.patch.adaptive_cfg}')

        fooocus_modules.patch.sharpness = sharpness
        print(f'[参数] 锐度 Sharpness = {fooocus_modules.patch.sharpness}')

        fooocus_modules.patch.positive_adm_scale = advanced_parameters.adm_scaler_positive
        fooocus_modules.patch.negative_adm_scale = advanced_parameters.adm_scaler_negative
        fooocus_modules.patch.adm_scaler_end = advanced_parameters.adm_scaler_end
        print(f'[参数] ADM Scale = '
              f'{fooocus_modules.patch.positive_adm_scale} : '
              f'{fooocus_modules.patch.negative_adm_scale} : '
              f'{fooocus_modules.patch.adm_scaler_end}')
        ...
        if input_image_checkbox:
            ...
            if current_tab == 'ip' or \
                ...
                if len(cn_tasks[flags.cn_canny]) > 0:
                    controlnet_canny_path = fooocus_modules.config.downloading_controlnet_canny()
                if len(cn_tasks[flags.cn_cpds]) > 0:
                    controlnet_cpds_path = fooocus_modules.config.downloading_controlnet_cpds()
                if len(cn_tasks[flags.cn_ip]) > 0:
                    clip_vision_path, ip_negative_path, ip_adapter_path = fooocus_modules.config.downloading_ip_adapters('ip')
            ...
            try:
                ...
                for x in imgs:
                    d = [
                        ...
                        ('ADM导向', str((
                            fooocus_modules.patch.positive_adm_scale,
                            fooocus_modules.patch.negative_adm_scale,
                            fooocus_modules.patch.adm_scaler_end))),
